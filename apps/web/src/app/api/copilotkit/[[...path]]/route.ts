...
import { readAssistantBrief } from "agent-core/lore";
import { loreWebPrompt } from "@/lib/lore-web-prompt";

// Web writes use /api/followups after a browser approval. Never expose raw MCP writes here.
const runtime = new CopilotRuntime({
  agents: async () => ({
    default: makeAgent(randomUUID(), { workplace: false, prompt: loreWebPrompt(await readAssistantBrief()) }),
  }),
});
